...
vi.mock('react-router-dom', async () => {
  ...
  return {
    ...actual,
    useLocation: vi.fn(() => ({ pathname: '/docs', search: '', hash: '', state: null, key: 'test' })),
  }
})
...
describe('useRoutes', () => {
  const mockRoutes = [
    { path: '/docs', filePath: '/docs/index.md', locale: 'en', version: 'v1', title: 'Home' },
    { path: '/docs/guide', filePath: '/docs/guide.md', locale: 'en', version: 'v1', title: 'Guide' },
    { path: '/es/docs', filePath: '/docs/index.md', locale: 'es', version: 'v1', title: 'Inicio' },
    { path: '/docs/v2', filePath: '/docs/v2.md', locale: 'en', version: 'v2', title: 'V2' },
  ]
  ...
  it('should return all routes when no i18n or versions configured', async () => {
    ...
    const { useBoltdocsContext } = await import('@/client/store/boltdocs-context')
    const { useRoutes } = await import('@/client/hooks/use-routes')

    vi.mocked(useRoutesContext).mockReturnValue({ routes: mockRoutes })
    vi.mocked(useConfig).mockReturnValue({})
    vi.mocked(ReactRouter.useLocation).mockReturnValue({ pathname: '/docs', search: '', hash: '', state: null, key: 'test' })
  ...
  it('should filter routes by current locale from store', async () => {
    ...
    vi.mocked(ReactRouter.useLocation).mockReturnValue({ pathname: '/es/docs', search: '', hash: '', state: null, key: 'test' })
    vi.mocked(useBoltdocsContext).mockReturnValue({
      hasHydrated: true,
      currentLocale: 'es',
      currentVersion: undefined,
  ...
  it('should filter routes by current version from store', async () => {
    ...
    vi.mocked(useRoutesContext).mockReturnValue({ routes: mockRoutes })
    vi.mocked(useConfig).mockReturnValue({
      versions: { defaultVersion: 'v1', versions: [{ label: 'v1', path: 'v1' }, { label: 'v2', path: 'v2' }] },
    })
    vi.mocked(ReactRouter.useLocation).mockReturnValue({ pathname: '/docs/v2', search: '', hash: '', state: null, key: 'test' })
    vi.mocked(useBoltdocsContext).mockReturnValue({
      hasHydrated: true,
      currentLocale: undefined,
      currentVersion: 'v2',
  ...
  it('should use default version from config when no version set', async () => {
    ...
    vi.mocked(useRoutesContext).mockReturnValue({ routes: mockRoutes })
    vi.mocked(useConfig).mockReturnValue({
      versions: { defaultVersion: 'v2', versions: [{ label: 'v1', path: 'v1' }, { label: 'v2', path: 'v2' }] },
    })
    vi.mocked(ReactRouter.useLocation).mockReturnValue({ pathname: '/docs', search: '', hash: '', state: null, key: 'test' })
    vi.mocked(useBoltdocsContext).mockReturnValue({
      hasHydrated: false,
  ...
  it('should return undefined currentRoute when pathname does not match any route', async () => {
    ...
    vi.mocked(useRoutesContext).mockReturnValue({ routes: mockRoutes })
    vi.mocked(useConfig).mockReturnValue({})
    vi.mocked(ReactRouter.useLocation).mockReturnValue({ pathname: '/unknown-route', search: '', hash: '', state: null, key: 'test' })
    vi.mocked(useBoltdocsContext).mockReturnValue({
      hasHydrated: true,
      currentLocale: undefined,
      currentVersion: undefined,
