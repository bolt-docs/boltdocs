...
cli
  ...
  .option('--fix', 'Automatically fix broken internal links and sync translations')
  .option('--check-external', 'Verify external links (slower)')
  .option('--init', 'Initialize doctor.json with default configuration')
  .action(async (root: string, options: { fix?: boolean; checkExternal?: boolean; init?: boolean }) => {
