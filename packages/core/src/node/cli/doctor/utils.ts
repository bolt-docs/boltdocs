...
import { type DoctorConfig, DEFAULT_DOCTOR_CONFIG, type DoctorContext } from './types'
import * as ui from '../ui'

export function getSeverity(ctx: DoctorContext, type: string, defaultLevel: 'high' | 'warning' | 'low'): 'high' | 'warning' | 'low' | 'off' {
...
export const fileCache = new Map<string, Promise<{ raw: string; data: Record<string, any>; content: string }>>()

export function getFileData(filePath: string): Promise<{ raw: string; data: Record<string, any>; content: string }> {
...
export async function generateLinkTree(docsDir: string, root: string = process.cwd(), config?: BoltdocsConfig, existingFiles?: string[]): Promise<any> {
