...
import { type DoctorContext, type DoctorIssue, DEFAULT_DOCTOR_CONFIG } from './types'
import { generateLinkTree, loadDoctorConfig, getFileData, backupFile } from './utils'
...
export async function doctorAction(root: string = process.cwd(), options: { fix?: boolean; checkExternal?: boolean; init?: boolean } = {}) {
  ...
  try {
    ...
    if (!fs.existsSync(docsDir)) {
      if (reportFormat === 'pretty') ui.error(`Docs dir not found at ${docsDir}`)
      process.exit(1)
    }
    ...
    const routeIndexWithSlash = new Set(linkTree.routes.map((r: string) => r.endsWith('/') ? r : r + '/'))
    const routeIndexWithoutSlash = new Set(linkTree.routes.map((r: string) => r.endsWith('/') ? r.slice(0, -1) : r))
    ...
    // 2. Reporting
    const reportData = {
      summary: { total: issues.length, high, warning, low, fixed: fixedCount, duration },
