...
import { visitNodes, parseMetaString, SKIP } from '@bdocs/unist-utils'
...
export function rehypeShiki(config?: BoltdocsConfig) {
  ...
  return async (tree: HastNode) => {
    ...
    visitNodes<ElementNode>(
      tree as unknown as Parameters<typeof visitNodes>[0],
      MDX_NODES.ELEMENT,
      (preNode) => {
        ...
        try {
          ...
          if (preElement && preElement.type === 'element') {
            ...
            preNode.properties = {
              ...(preNode.properties || {}),
              ...((preElement as unknown as ElementNode).properties || {}),
