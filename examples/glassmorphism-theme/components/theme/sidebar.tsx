...
export const CustomSidebar = ({ routes }: any) => {
  ...
  const sidebarContent = (
    <>
      ...
      {groups.map((group) => (
        <Sidebar.Group key={group.title} title={group.title} className="mb-8">
  ...
  return (
    <>
      <Sidebar className="boltdocs-sidebar">
        <Sidebar.Content>{sidebarContent}</Sidebar.Content>
      </Sidebar>
...
function SidebarRouteItem({ route, activePath }: any) {
  ...
  const isCurrent =
    activePath ===
    (localizedHref.endsWith('/') && localizedHref.length > 1
  ...
  return (
    <Sidebar.Link
      ...
      className={cn(
        ...
        isCurrent &&
          '!bg-primary-500/10 !text-primary-400 !border-l-2 !border-primary-500 !rounded-none',
      )}
    />
  )
}
