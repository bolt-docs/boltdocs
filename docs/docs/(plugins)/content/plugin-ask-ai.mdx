...
flowchart LR
  ...
  B -->|POST /api/ask-ai| C[Server middleware or serverless handler]
...
- **`<AskAiDialog />`** — optional sidebar panel for manual composition on `xl:` viewports (≥ 1280 px).
...
1. Resolves rendered page content server-side in Vite/preview. Serverless adapters accept pre-extracted `{ page, content }` only with `allowClientContext: true` or an authorized `secretKey`.
...
| **`persona`** | `string` (≤ 4 000 chars) | `undefined` | Custom identity/tone for the assistant. Prepended to the default prompt inside `<<<PERSONA>>>` markers — it shapes *how* the assistant talks, but cannot weaken the scoping/refusal rules. Ignored when `systemPrompt` (or `systemPrompts[provider]`) fully overrides. |
| **`temperature`** | `number` (0–2) | `0.3` | Sampling temperature. `0` = deterministic (best for doc Q&A). |
| **`topP`** | `number` (0–1) | `1` | Nucleus-sampling cutoff. `1` = no restriction. |
| **`title`** | `string` (≤ 80) | `'Ask Assistant'` | Header title of the chat widget (bubble and dialog). |
| **`placeholder`** | `string` (≤ 120) | `'Ask about this page…'` | Composer placeholder. |
| **`emptyTitle`** | `string` (≤ 80) | `'How can I help you today?'` | Greeting shown before the first message (bubble only). |
| **`emptyDescription`** | `string` (≤ 240) | `'Ask anything about the current documentation page…'` | Sub-text under the greeting. |
| **`buttonTooltip`** | `string` (≤ 80) | `'Ask AI assistant'` | Tooltip/aria-label of the floating launcher. |
| **`composerHint`** | `string` (≤ 120) | `'Enter to send · Shift+Enter for a new line'` | Hint under the input. |
...
### Automatic mounting and manual composition

The plugin automatically mounts one `<AskAiBubble />` through the `floating:after` slot. To compose the UI yourself, set `autoInject: false` and import the client components in your `layout.tsx`:
...
Set `autoInject: false` when you want to compose `<AskAiBubble />` and/or `<AskAiDialog />` directly in your `layout.tsx`:
...
The plugin mounts `<AskAiBubble />` automatically through `floating:after`. Set `autoInject: false` only when you need manual placement in your layout. Here are the recommended patterns:
...
- The plugin auto-mounts `<AskAiBubble />` through the `floating:after` slot by default.
- Set `autoInject: false` only when you are rendering the client component manually in your `layout.tsx`.
