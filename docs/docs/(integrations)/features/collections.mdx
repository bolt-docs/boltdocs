...
Returns an array of all posts in a specific collection.
...
Returns a specific post by its slug (or full path) from a collection, or \`undefined\` if not found.
...
Returns the most recent posts of a collection. Default count is 5.
