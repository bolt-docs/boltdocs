...
| Lang icons in MDX code blocks | `icons-dev.tsx` renamed to `lang-icons.tsx` and lazy-loaded inside `useCodeBlock` | End users (no action) | None — pages without code blocks now ship zero bytes of these icons; pages with code blocks still get the same icons |
...
// ✅ After 3.2.0 — language icons
import { langIconsMap } from 'boltdocs/client/mdx/lang-icons'  // lazy
```

If you only imported the language icons, prefer the public `langIconsMap` from `'@bdocs/parser/mdx/lang-icons'` (lazy `import()` from within your `useCodeBlock`-equivalent hook). For the social icons, use the new `icons-prod.tsx` exports.
...
- **MDX code blocks render the same lang icons.** The icons load lazily in 3.2.0, but on document complete they look identical to 3.1.x.
...
The only thing that *visibly* changes is the icon color flash during the brief hydration window on pages with MDX code blocks: SSR/initial-hydration renders the gray `File` icon, then sub-frame later the colored language icon swap happens. Same visual as 3.1.x on settled render.
...
### "Can I opt out of the lazy-load for lang icons?"

No — the lazy load is a single-line `import('./lang-icons')` inside `useCodeBlock`. There's no flag; pages without code blocks simply skip the chunk load by doing nothing. If you need a single-page bundle that includes all icons, build a custom layout: import the icons directly from `@bdocs/parser/mdx/lang-icons` and replace `useCodeBlock`'s `LangIcon` slot.
...
No. In fact, on pages without MDX code blocks, the bundle goes **down** by ~17 KB (the language icons never load). Pages with code blocks still load the same icons — they just live in a `lang-icons-*.js` chunk instead of inline. See the [package weight blog section](/docs/blog/boltdocs-3.2.0#package-weight-reduction) for the full before/after table.
