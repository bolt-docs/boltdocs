...
Add project-specific module aliases without replacing the aliases Boltdocs uses internally:
...
The core keeps aliases such as `boltdocs/client`, `boltdocs/entry`, and `use-sync-external-store` configured automatically. The `aliases` option accepts the same object or array formats as Vite. Advanced Vite settings can still be provided through `vite`; `vite.resolve.alias` is merged with `aliases` instead of replacing the framework aliases. Framework aliases take precedence when the same alias is declared twice.
...
| **`codeTheme`** | [`CodeThemeConfig`](#codethemeconfig) | — | Configure the syntax highlighting theme. **Deprecated** — use [`codeHighlighting`](#codehighlightconfig) instead. The value is the same accepted by `codeHighlighting.theme`. |
| **`codeHighlighting`** | [`CodeHighlightConfig`](#codehighlightconfig) | — | Syntax highlighting engine and theme. Engine-agnostic: defaults to the built-in Shiki engine and accepts any custom `CodeHighlighterAdapter`. |
...
The engine-agnostic way to configure syntax highlighting. The core never talks
to a highlighting library directly — it renders through a
`CodeHighlighterAdapter` resolved from a registry, so **Shiki is just the
default engine** and any highlighter can be plugged in without touching the
core.

| Property | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| **`engine`** | `string \| CodeHighlighterAdapter \| (api) => CodeHighlighterAdapter` | `'shiki'` | The highlighting engine. A string is a registry id (`'shiki'` or any engine registered by a plugin). Pass an adapter instance or a factory to inject a custom engine. |
| **`theme`** | `string \| { light; dark }` | `{ light: 'github-light', dark: 'github-dark' }` | Theme name or light/dark pair. When an object is given, the rendered blocks carry `data-theme-mode="dual"` and use `--shiki-light`/`--shiki-dark` CSS variables. |
| **`options`** | `Record<string, unknown>` | `{}` | Engine-specific options, passed through to the adapter. |

Options for the built-in Shiki engine:

| Option | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| **`regexEngine`** | `'oniguruma' \| 'javascript'` | `'oniguruma'` | `'javascript'` swaps Oniguruma WASM for Shiki's native JS regex engine, cutting highlighter startup from ~2.5s to ~200ms at a small cost in regex fidelity for exotic grammars. Useful for faster dev-server starts. |
...
Plugins expose a highlighting engine through the `codeHighlighter` field. The
adapter is registered under the plugin's name, so `engine` can select it by id:

```ts
import { defineConfig, definePlugin, type CodeHighlighterAdapter } from 'boltdocs'

const prismEngine: CodeHighlighterAdapter = {
  name: 'prism',
  getOptions(lang, meta) {
    return { lang, theme: 'one-light' }
  },
  async initialize() {
    const prism = await import('your-prism-wrapper')
    return {
      codeToHast: (code, options) => prism.toHast(code, options),
      codeToHtml: async (code, options) => prism.toHtml(code, options),
    }
  },
...
const prismPlugin = definePlugin({
  name: 'my-prism',
  codeHighlighter: prismEngine,
})

export default defineConfig({
  plugins: [prismPlugin()],
  theme: {
    codeHighlighting: { engine: 'my-prism' },
  },
})
```

An engine can also be passed inline as an adapter instance or factory without
...
codeHighlighting: {
  engine: {
    name: 'prism',
    getOptions: (lang, meta) => ({ lang }),
    initialize: () => import('./adapter').then((m) => m.runtime),
  },
},
```

For more on the plugin system, see [Plugins](/docs/plugins).
