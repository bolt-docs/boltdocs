...
2. **Disk Cache** — Persisted to `.boltdocs/` directory for reuse across restarts
...
| :--- | :--- | :--- | :--- |
| **`BOLTDOCS_CACHE_DIR`** | `string` | `.boltdocs` | Directory for all cache files |
...
│   └── config/            ← Cached config resolution
├── docs/
└── boltdocs.config.ts
```
