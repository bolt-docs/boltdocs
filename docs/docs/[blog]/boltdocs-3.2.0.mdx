...
| `icons-dev.tsx` language icon blob in the initial chunk | **44 KB on every page** | **0 KB** on pages without code blocks; **~17 KB** on pages that render them | Lang icons moved to `lang-icons.tsx` and loaded via `import('./lang-icons')` inside `useCodeBlock`. Appears as a separate `lang-icons-*.js` chunk in the build output. |
...
3. **Lang icons in MDX code blocks** — twelve language icons (TypeScript, JavaScript, React, JSON, CSS, HTML, Markdown, Shell, YAML, Rust, TOML, CSV) were always eagerly bundled in `icons-dev.tsx`. They are now in a separate `lang-icons.tsx` module loaded via `React.lazy` inside the `useCodeBlock` hook. Pages without code blocks ship zero bytes of these icons; pages with code blocks load them on first render. During SSR and the brief hydration window, the generic `File` icon is rendered — same as before, no visual flash beyond a sub-frame swap from gray to colored icons.
...
Read the full audit in **[Upgrading to 3.2.0 → Semi-breaking changes](/docs/guides/upgrading-3-2)**.
...
See **[Upgrading to 3.2.0 → Semi-breaking changes](/docs/guides/upgrading-3-2)** for the full audit, before/after `package.json` snippets, and platform-specific notes.
