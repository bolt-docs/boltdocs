...
La bandera `--turbo` que introdujimos en 3.0.0 fue el primer paso. La extracción de CSS crítico impulsada por Zig (`@bdocs/zig-critters`) procesa 300 archivos HTML en menos de un segundo. Eso probó el concepto: mover operaciones críticas del build a código nativo paga inmediatamente.
...
`computeClientCodeHash()` escanea recursivamente todo el monorepo con `fs.statSync()` síncrono — miles de syscalls en cada build. Vamos a cachear el hash y solo re-escanear cuando `packages/` realmente cambie.
...
Reemplazar `child_process.execFile()` (pipe → stdout → JSON → parse) con una llamada directa N-API. Sin serialización, sin I/O de filesystem, strings zero-copy de Zig a V8.
...
## Cómo estamos construyendo esto
...
Los resultados hablan por sí mismos: el análisis identificó 12 cuellos de botella distintos, clasificados por impacto, con rutas de archivo y números de línea específicos. Ese es el nivel de precisión con el que estamos trabajando.
