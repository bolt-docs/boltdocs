...
| `icons-dev.tsx` (blob de iconos de lenguajes) en el chunk inicial | **44 KB en cada página** | **0 KB** en páginas sin code blocks; **~17 KB** en páginas que los renderizan | Los iconos de lenguajes se movieron a `lang-icons.tsx` y se cargan vía `import('./lang-icons')` dentro de `useCodeBlock`. Aparecen como un chunk separado `lang-icons-*.js`. |
...
3. **Iconos de lenguajes en code blocks MDX** — los doce iconos de lenguajes (TypeScript, JavaScript, React, JSON, CSS, HTML, Markdown, Shell, YAML, Rust, TOML, CSV) siempre estuvieron bundleados eager en `icons-dev.tsx`. Ahora están en un módulo separado `lang-icons.tsx` cargado vía `React.lazy` dentro del hook `useCodeBlock`. Las páginas sin code blocks envían cero bytes de estos iconos; las páginas con code blocks los cargan en el primer render. Durante el SSR y la breve ventana de hidratación, se muestra el ícono genérico `File` — sin flash visual más allá de un swap de sub-frame de gris a color.
...
Leé la auditoría completa en **[Actualizar a 3.2.0 → Cambios semi-breaking](/es/guides/upgrading-3-2)**.
...
Consultá **[Actualizar a 3.2.0 → Cambios semi-breaking](/es/guides/upgrading-3-2)** para la auditoría completa, snippets de `package.json` antes/después y notas por plataforma.
