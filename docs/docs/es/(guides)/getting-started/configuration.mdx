...
| **`plugins`** | `BoltdocsPlugin[]` | `[]` | Un array de plugins de Boltdocs. Consulta la pestaña [Plugins](/docs/plugins) para la referencia completa del sistema de plugins. |
| **`theme`** | [`ThemeConfig`](#themeconfig) | — | Controla la apariencia visual, navegación, sidebar y opciones de visualización. |
| **`seo`** | [`SeoConfig`](#seoconfig) | — | Controla el comportamiento de indexación y generación de thumbnails Open Graph. |
| **`robots`** | [`RobotsConfig`](#robotsconfig) | — | Genera `robots.txt` y enlaza tu sitemap. |
| **`integrations`** | [`IntegrationsConfig`](#integrationsconfig) | — | Habilita integraciones de terceros como Google Analytics 4 y Google Tag Manager. |
| **`i18n`** | [`I18nConfig`](#i18nconfig) | — | Habilita documentación multilingüe con enrutamiento basado en carpetas por locale. |
| **`versions`** | [`VersionsConfig`](#versionsconfig) | — | Habilita documentación versionada lado a lado. |
| **`collections`** | [`CollectionsConfig`](#collectionsconfig) | — | Configura el sistema de colecciones dinámicas (blog) para agrupar publicaciones relacionadas. |
...
| **`security`** | [`SecurityConfig`](#securityconfig) | — | Configura headers de seguridad HTTP y reglas CSP. |
...
| **`tabs`** | `TabConfig[]` | — | Franja de pestañas horizontal sobre el sidebar. Consulta [Enrutamiento por Sistema de Archivos](/docs/guides/getting-started/file-routing). |
