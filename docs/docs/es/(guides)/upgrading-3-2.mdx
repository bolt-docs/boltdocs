---
title: "Actualizar a Boltdocs 3.2.0 — Cambios semi-breaking"
sidebarPosition: 4
description: "La auditoría completa de cambios en peer dependencies, dependencias movidas y comportamiento runtime en boltdocs 3.2.0. Incluye recetas de migración, manejo de CI / lockfile-strict y FAQ."
badge: Migración
---
...
| Iconos de lenguajes en code blocks MDX | `icons-dev.tsx` renombrado a `lang-icons.tsx` y lazy-loaded dentro de `useCodeBlock` | Usuarios finales (sin acción) | Ninguna — páginas sin code blocks ahora envían cero bytes de estos iconos; páginas con code blocks siguen recibiendo los mismos iconos |
...
// ✅ Después de 3.2.0 — íconos de lenguajes
import { langIconsMap } from 'boltdocs/client/mdx/lang-icons'  // lazy
```

Si solo importabas los iconos de lenguajes, preferí el `langIconsMap` público desde `'@bdocs/parser/mdx/lang-icons'` (lazy `import()` desde dentro de tu hook equivalente a `useCodeBlock`). Para los iconos sociales, usá los exports nuevos de `icons-prod.tsx`.
...
- **Los code blocks MDX renderizan los mismos íconos de lenguajes.** Los íconos se cargan lazy en 3.2.0, pero en document complete se ven idénticos a 3.1.x.
...
Lo único que *visiblemente* cambia es el flash de color del ícono durante la breve ventana de hidratación en páginas con code blocks MDX: SSR/hidratación inicial renderiza el ícono `File` gris, después en sub-frame sucede el swap al ícono de lenguaje coloreado. Mismo visual que 3.1.x en el render asentado.
...
### "¿Puedo desactivar el lazy-load de los íconos de lenguajes?"

No — el lazy load es un `import('./lang-icons')` de una sola línea dentro de `useCodeBlock`. No hay flag; las páginas sin code blocks se saltean la carga del chunk simplemente no haciendo nada. Si necesitás un bundle de una sola página que incluya todos los íconos, armá un layout custom: importá los íconos directamente desde `@bdocs/parser/mdx/lang-icons` y reemplazá el slot `LangIcon` de `useCodeBlock`.
...
No. De hecho, en páginas sin code blocks MDX, el bundle **baja** ~17 KB (los íconos de lenguajes nunca se cargan). Las páginas con code blocks siguen cargando los mismos íconos — simplemente viven en un chunk `lang-icons-*.js` en lugar de inline. Consultá la [sección de peso del paquete en el blog](/es/blog/boltdocs-3.2.0#reducción-del-peso-del-paquete) para la tabla completa de antes/después.
