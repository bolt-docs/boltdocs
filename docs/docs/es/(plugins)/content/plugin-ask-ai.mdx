...
flowchart LR
  ...
  B -->|POST /api/ask-ai| C[Middleware del servidor o handler serverless]
...
- **`<AskAiDialog />`** — panel lateral opcional para composición manual en viewports `xl:` (≥ 1280 px).
...
1. Resuelve el contenido de la página en el servidor en Vite/preview. Los adaptadores serverless solo aceptan `{ page, content }` con `allowClientContext: true` o un `secretKey` autorizado.
...
| **`persona`** | `string` (≤ 4 000 caracteres) | `undefined` | Identidad/tono personalizado del asistente. Se antepone al prompt por defecto dentro de marcadores `<<<PERSONA>>>` — moldea *cómo* habla, pero no puede debilitar las reglas de alcance/rechazo. Se ignora si `systemPrompt` (o `systemPrompts[provider]`) lo sobrescribe por completo. |
| **`temperature`** | `number` (0–2) | `0.3` | Temperatura de muestreo. `0` = determinista (recomendado para Q&A de docs). |
| **`topP`** | `number` (0–1) | `1` | Corte de nucleus sampling. `1` = sin restricción. |
| **`title`** | `string` (≤ 80) | `'Ask Assistant'` | Título del header del widget (burbuja y diálogo). |
| **`placeholder`** | `string` (≤ 120) | `'Ask about this page…'` | Placeholder del compositor. |
| **`emptyTitle`** | `string` (≤ 80) | `'How can I help you today?'` | Saludo antes del primer mensaje (solo burbuja). |
| **`emptyDescription`** | `string` (≤ 240) | `'Ask anything about the current documentation page…'` | Texto bajo el saludo. |
| **`buttonTooltip`** | `string` (≤ 80) | `'Ask AI assistant'` | Tooltip/aria-label del lanzador flotante. |
| **`composerHint`** | `string` (≤ 120) | `'Enter to send · Shift+Enter for a new line'` | Pista bajo el input. |
...
## Montaje Automático y Composición Manual

El plugin monta automáticamente `<AskAiBubble />` mediante el slot `floating:after`. Para componer la UI manualmente, configura `autoInject: false` y añade los componentes que necesites en tu `layout.tsx`:
...
- `autoInject: true` (predeterminado) monta `<AskAiBubble />` globalmente mediante el slot `floating:after`. Si escribiste componentes manualmente en tu layout, usa `autoInject: false` para evitar duplicados.
