...
El plugin genera los feeds mediante el hook de ciclo de vida `build:generate`, que el pipeline de compilación ejecuta después de que la salida SSG se escribe en el disco (con `afterBuild` como fallback de compatibilidad):
...
| :--- | :--- | :--- |
| RSS | `/rss/rss-{locale}.xml` | `/rss/rss-{locale}.xml` |
