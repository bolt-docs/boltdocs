...
### 1. Banner de Título
Agrega `title="file_name.extension"` para mostrar un encabezado estilizado que contiene el nombre del archivo y un icono de idioma coincidente:
...
| **`LangIcon`** | `ComponentType<{ size?: number }>` | El componente de icono React resuelto que coincide con el idioma analizado del bloque de código. |
