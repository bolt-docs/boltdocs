...
import { injectHtmlMeta } from '../packages/core/src/node/plugin/html'
import { useAnalytics } from '../packages/core/src/client/hooks/use-analytics'
...
// Mock useLocation
vi.mock('../packages/core/src/client/hooks/use-location', () => ({
  useLocation: vi.fn(() => ({ pathname: '/', search: '' })),
}))

describe('Analytics Integration', () => {
  const baseHtml = '<!doctype html><html><head></head><body><div id="root"></div></body></html>'
  ...
  describe('Node-side: injectHtmlMeta', () => {
    ...
    it('should inject GA4 script with all options', () => {
      ...
      expect(result).toContain('https://www.googletagmanager.com/gtag/js?id=G-XXXXX')
      expect(result).toContain("gtag('set', 'ip', true);")
      expect(result).toContain("gtag('config', 'G-XXXXX', {send_page_view: false}, {'cookie_flags': 'SameSite=None;Secure'});")
    })
    ...
    it('should inject GTM script with dataLayerName and preview', () => {
      ...
      expect(result).toContain('https://www.googletagmanager.com/ns.html?id=GTM-XXXXX')
    })
  })
  ...
  describe('Client-side: useAnalytics Hook', () => {
    ...
    it('should use dataLayer when gtag is missing but dataLayer exists', () => {
      ...
      expect(dataLayerSpy).toHaveBeenCalledWith(expect.objectContaining({
        event: 'click',
    ...
    it('should track external links', () => {
      ...
      expect(gtagSpy).toHaveBeenCalledWith('event', 'external_link', expect.objectContaining({
    ...
    it('should track downloads', () => {
      ...
      clickHandlers.forEach((handler: any) => handler(mockEvent))
      expect(gtagSpy).toHaveBeenCalledWith('event', 'file_download', expect.objectContaining({
    ...
    it('should respect excludePatterns', () => {
      ...
      expect(gtagSpy).not.toHaveBeenCalledWith('event', 'external_link', expect.anything())
    })
  })
})
